...
export const AGENT_IDENTITY_CODE_OID = "1.2.156.3088" as const;
export const CURRENT_IDENTITY_CODE_VERSION = "1" as const;

const NODE_RE = /^[0-9A-Z]+$/i;

function validateNode(value: string, name: string, maxLength: number): void {
  if (!value || value.length > maxLength || !NODE_RE.test(value)) {
    throw new Error(`${name} must be 1-${maxLength} base36 characters`);
  }
}

export function formatIdentityCode(parts: Omit<AgentIdentityCodeParts, "oid" | "version"> & Partial<Pick<AgentIdentityCodeParts, "oid" | "version">>): AgentIdentityCode {
  const normalized: AgentIdentityCodeParts = {
...
export function parseIdentityCode(code: AgentIdentityCode): AgentIdentityCodeParts {
  ...
  const parts: AgentIdentityCodeParts = {
...
export function validateIdentityCode(code: AgentIdentityCode): boolean {
  try {
...
export function validateIdentityCodeParts(parts: AgentIdentityCodeParts): void {
